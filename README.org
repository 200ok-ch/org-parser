...
ANTLR returns a structural AST. This split is intentional for now.

Current parser layering:

- grammar layer: =resources/antlr/*.g4=
- runtime adapters: =src/org_parser/antlr/parser.clj= and
  =src/org_parser/antlr/parser.cljs=
- normalization/post-processing: =src/org_parser/antlr/postprocess.cljc=
- public parser contract: =src/org_parser/parser.cljc=
- transform layer: =src/org_parser/transform.cljc=

Current roadmap focus:

- keep parser contract stable and tested across CLJ and CLJS
- minimize runtime drift risks first
- migrate high-value direct parsing paths into grammar where practical
- validate parser and parse+transform performance on representative fixtures
...
For maintainers, parser layering and migration priorities are
summarized in the Architecture section above.
